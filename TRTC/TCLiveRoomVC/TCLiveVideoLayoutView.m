...
@property(nonatomic,assign)int bianmaw;
@property(nonatomic,assign)int bianmah;
...
- (NSArray *)customLayoutFrames{
    ...
    if (TCLiveRoomVideoLayoutStyle_1v1 == _layoutStyle) {
        [frames removeAllObjects];
        CGRect frame1 = CGRectMake(0, 100, self.frame.size.width/2, self.frame.size.height -200);
        CGRect frame2 = CGRectMake(self.frame.size.width/2, 100, self.frame.size.width/2, self.frame.size.height-200);
        [frames addObject:[NSValue valueWithCGRect:frame1]];
        [frames addObject:[NSValue valueWithCGRect:frame2]];
    }
    else if (TCLiveRoomVideoLayoutStyle_1v2 == _layoutStyle) {
        [frames removeAllObjects];
    ...
    else if (TCLiveRoomVideoLayoutStyle_1v2_gezi == _layoutStyle) {
        [frames removeAllObjects];
        CGRect frame1 = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height - self.frame.size.width/2 - 1);
        CGRect frame2 = CGRectMake(0, self.frame.size.height - (self.frame.size.width/2 - 1), self.frame.size.width/2-1, self.frame.size.width/2 - 1);
        CGRect frame3 = CGRectMake(self.frame.size.width/2+1, self.frame.size.height - (self.frame.size.width/2 - 1), self.frame.size.width/2-2, (self.frame.size.width/2 - 1));
        [frames addObject:[NSValue valueWithCGRect:frame1]];
        [frames addObject:[NSValue valueWithCGRect:frame2]];
        [frames addObject:[NSValue valueWithCGRect:frame3]];
    }
    else if (TCLiveRoomVideoLayoutStyle_1v3 == _layoutStyle) {
        [frames removeAllObjects];
        int smallViewWidth = ((self.frame.size.height - 20  - 150 - 10 *2)/3)*(3.0/4.0);
        int smallViewHeight = (self.frame.size.height - 20  - 150 - 10 *2)/3;
        CGRect frame1 = self.bounds;
    ...
    else if (TCLiveRoomVideoLayoutStyle_1v3_gezi == _layoutStyle) {
        [frames removeAllObjects];
        CGRect frame1 = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height - self.frame.size.width/3 - 1);
        CGRect frame2 = CGRectMake(0, self.frame.size.height - (self.frame.size.width/3 - 1), self.frame.size.width/3-1, self.frame.size.width/3 - 1);
        CGRect frame3 = CGRectMake(self.frame.size.width/3+1, self.frame.size.height - (self.frame.size.width/3 - 1), self.frame.size.width/3-2, (self.frame.size.width/3 - 1));
        CGRect frame4 = CGRectMake((self.frame.size.width * 2)/3 + 1, self.frame.size.height - (self.frame.size.width/3 - 1), self.frame.size.width/3 -1, (self.frame.size.width/3 - 1));
        [frames addObject:[NSValue valueWithCGRect:frame1]];
        [frames addObject:[NSValue valueWithCGRect:frame2]];
        [frames addObject:[NSValue valueWithCGRect:frame3]];
        [frames addObject:[NSValue valueWithCGRect:frame4]];
...
- (void)onFirstFrameRecved:(int)width height:(int)height identifier:(NSString *)identifier srcType:(avVideoSrcType)srcType;{
    int dateTime = [[NSDate  date] timeIntervalSinceDate:self.startTime];
    [ILiveLogReport reportEvent:1202 retTime:dateTime eventTime:0 roomId:[self.roomInfo[@"roomID"] intValue] extInfo:nil code:0 msg:nil];
}
...
- (void)showElementVideoInfoWithVideoFrame:(QAVVideoFrame *)frame{
    ...
    NSString *bianma = [[[[qualityParams componentsSeparatedByString:@"视频编码:"] lastObject] componentsSeparatedByString:@"新的参考帧结构参数"] firstObject];
    self.bianmaw = [[[[[bianma componentsSeparatedByString:@" W="] lastObject] componentsSeparatedByString:@","] firstObject] intValue];
    self.bianmah = [[[[[bianma componentsSeparatedByString:@" H="] lastObject] componentsSeparatedByString:@","] firstObject] intValue];
