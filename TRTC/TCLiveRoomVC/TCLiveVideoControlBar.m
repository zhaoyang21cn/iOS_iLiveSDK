...
- (UIButton *)logBtn{
    if (!_logBtn) {
        _logBtn = [self createCustomControlBtn:@"日志" withImage:[UIImage imageNamed:@"log.png"] selectedImage:[UIImage imageNamed:@"log2.png"]];
        [_logBtn addTarget:self action:@selector(logBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    _logBtn.frame = CGRectMake(self.frame.size.width/2 +  CONTROLBAR_BUTTON_WIDTH * 1.5, 0, CONTROLBAR_BUTTON_WIDTH, self.frame.size.height);
    return _logBtn;
}
...
- (UIButton *)feedBackBtn{
    ...
    _feedBackBtn.frame = CGRectMake(self.frame.size.width/2 + CONTROLBAR_BUTTON_WIDTH * 2.5, 0, CONTROLBAR_BUTTON_WIDTH, self.frame.size.height);
    return _feedBackBtn;
}
...
- (void)chatBtnClick:(UIButton *)sender{
    ...
    [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"chat" code:0 msg:nil];
}
//切换摄像头
- (void)switchCameraClick:(UIButton *)sender{
    ...
    [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"switch_camera" code:0 msg:nil];
}
//美颜
- (void)beautyBtnClick:(UIButton *)sender{
    ...
    if ([_delegate respondsToSelector:@selector(beautyBtnClick:)]) {
     ...
     [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"beauty_face" code:0 msg:nil];
}
...
- (void)voiceBtnClick:(UIButton *)sender{
    if (!sender.selected) {
        ...
        sender.selected = NO;
         [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"disable_mic" code:0 msg:nil];
    }
...
- (void)changeRoleBtnClick:(UIButton *)sender{
    ...
    [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"role" code:0 msg:nil];
}
//日志
- (void)logBtnClick:(UIButton *)sender{
    ...
    if ([_delegate respondsToSelector:@selector(logBtnClick:)]) {
     ...
     [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"log" code:0 msg:nil];
}
//问题反馈
- (void)feedBackBtnClick:(UIButton *)sender{
    ...
    [ILiveLogReport reportEvent:1203 retTime:0 eventTime:0 roomId:0 extInfo:@"feedback" code:0 msg:nil];
}
//log展示开关
...
- (void)feedBackWithExt:(NSString *)ext{
    ...
    [ILiveLogReport reportEvent:1204 retTime:0 eventTime:0 roomId:0 extInfo:ext code:0 msg:nil];
}
//其他反馈输入
...
//    [alertController addAction:[UIAlertAction actionWithTitle:@"切换视频布局" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        if (self.style >5) {
//            self.style = 0;
//        }
//        [((TCLiveRoomViewController *)self.delegate).videoLayoutView setLayoutStyle:self.style];
//        self.style++;
//    }]];
