...
//推流服务器类型
typedef NS_ENUM(NSInteger, ILivePushSvrType)
{
    ILive_PushSvr_Type_Research = 0,//自研机房
    ILive_PushSvr_Type_Cloud,       //云支持(默认)
};
...
/** 推流server类型 */
@property (nonatomic, assign) ILivePushSvrType pushSvrType;

/** 录制文件ID，用户自定义这个id，用来在后台匹配录制文件 */
@property (nonatomic, assign) uint32_t recordId;
