...
@class ILiveCameraPreviewOption;
...
/**
 ...
 @param quitIm  YES:退出IM群组，NO:不退出IM群组
 @param succ  退出直播间成功回调
 @param fail 退出直播间失败回调
 */
- (void)quitRoomExt:(BOOL)quitIm succ:(TCIVoidBlock)succ failed:(TCIErrorBlock)fail;
...
/**
 ...
 @param fail 失败回调
 */
- (void)setCameraPreviewParam:(ILiveCameraPreviewOption *)option succ:(TCIVoidBlock)succ failed:(TCIErrorBlock)fail;

/**
 开启相机预览
 注：使用本接口打开相机，是没有上行权限的，只是在本地预览（与requestVideoAuth接口配合使用）
 注：注: MacOS端暂不支持(调用无效)
...
/**
 ...
 @param isEnable YES:开启视频上行权限 NO:关闭视频上行权限
 @param succ 成功回调
 @param fail 失败回调
 */
- (void)requestVideoAuth:(BOOL)isEnable succ:(TCIVoidBlock)succ failed:(TCIErrorBlock)fail;
...
/**
 ...
 @param bEnable 开启屏幕分享
 @param succ 成功回调
 @param fail 失败回调
 */
- (void)enableScreenShare:(BOOL)bEnable succ:(TCIVoidBlock)succ failed:(TCIErrorBlock)fail;

/**
 设置屏幕分享区域(暂不支持指定窗口分享)
 移动端下设置无效、未开启屏幕分享时设置无效

 @param rect 分享区域 最小宽192，最小高144,如果小于最小值，内部会自动转化成最小值
 */
- (void)setScreenShareRect:(CGRect)rect;
...
/**
 ...
 @param delegate 屏幕分享本地画面代理
 */
- (void)setLocalScreenVideoDelegate:(id<QAVLocalScreenVideoDelegate>)delegate;
#endif
